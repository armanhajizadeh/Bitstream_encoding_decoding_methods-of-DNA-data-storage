...
# Embedded CSV data as a string (replace this with your actual 7to8.csv content)
...
def load_embedded_mapping():
    ...
    """
    encoding_map = {}
    csv_file = io.StringIO(embedded_csv_data)
...
# Create a checkbox for using custom CSV file
use_custom_csv = st.checkbox("Use custom 7to8.csv file (optional)")

# Only show file uploader if checkbox is selected
...
if use_custom_csv:
    ...
    if uploaded_file:
        try:
            ...
            custom_encoding_map = {}
            with open("temp_7to8.csv", 'r', newline='') as csvfile:
                reader = csv.reader(csvfile)
                next(reader)  # Skip header
                for row in reader:
                    if len(row) == 2:
                        custom_encoding_map[row[0]] = row[1]
            
            st.success("Custom mapping loaded successfully!")
        except Exception as e:
            st.error(f"Error loading custom CSV: {e}")
...
if st.button("Encode to DNA"):
    if not user_input:
        ...
        try:
            # Use custom encoding map if provided, otherwise use embedded
            encoding_map = custom_encoding_map if custom_encoding_map else load_embedded_mapping()
...
# Add an expander with information about the app
with st.expander("About this App"):
    st.markdown("""
    This app converts text to DNA sequences using a specialized encoding technique:
    ...
    The encoding is optimized for DNA storage properties such as GC content balance.
    
    You can use the default embedded mapping or upload your own 7to8.csv file for custom mappings.
    """)